 ...
 *
 * Color scheme generation ported from the Xime project (GPL-3.0),
...
private const val SEED_LIGHT = 0xFF8F73E2
private const val SEED_CONTAINER_LIGHT = 0xFFEADDFF
private const val SEED_DARK = 0xFFD0BCFF
private const val SEED_CONTAINER_DARK = 0xFF4F378B
...
@Composable
fun TrimeSettingsTheme(
    darkTheme: Boolean = isSystemInDarkTheme(),
    content: @Composable () -> Unit,
) {
    val seed = if (darkTheme) Color(SEED_DARK) else Color(SEED_LIGHT)
    val container = if (darkTheme) Color(SEED_CONTAINER_DARK) else Color(SEED_CONTAINER_LIGHT)
